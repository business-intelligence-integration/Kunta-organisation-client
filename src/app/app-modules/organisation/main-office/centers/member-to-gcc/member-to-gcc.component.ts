...
@Component({
  selector: 'app-member-to-gcc',
  templateUrl: './member-to-gcc.component.html',
  styleUrls: ['./member-to-gcc.component.scss']
})
export class MemberToGccComponent implements OnInit {
  ...
  getCenter(){
    this.activatedRoute.queryParams.subscribe((params) => {
      this.centerService.getCenterById(params['id']).subscribe((res)=>{
        ...
        this.users = res.data.governanceAndCompensationCommittee;
      });
    })
  }
  ...
  addMemberToCenter(idCenter: number, idMember: number, idPost: number){
    this.isSaving = true;
    this.centerService.addToGcc(idCenter, idMember, idPost).subscribe(()=>{
      this.isSaving = false;
      this.getCenter();
    ...
    swalWithBootstrapButtons
      ...
      .then((result) => {
        if (result.isConfirmed) {
          this.centerService.removeFromGcc(this.idCenter, id).subscribe(
            () => {
              this.getCenter();
