...
@Component({
  selector: 'app-strategic-development-committee',
  templateUrl: './strategic-development-committee.component.html',
  styleUrls: ['./strategic-development-committee.component.scss']
})
export class StrategicDevelopmentCommitteeComponent implements OnInit {
  ...
  getAllMainOffice(){
    this.mainOfficeService.findAllOffices().subscribe((res)=>{
      ...
      this.users = res.data[0].strategicDevelopmentCommittee;
    })
  }
  ...
  onAddMember(){
    ...
    this.addMemberToaddToSdc(this.idMainOffice, formValue.id, formValue.idFonction);
  }

  addMemberToaddToSdc(idMainOffice: number, idMember: number, idFonction: number){
    this.isSaving = true;
    this.mainOfficeService.addMemberToSdc(idMainOffice, idMember, idFonction).subscribe(()=>{
      this.isSaving = false;
      this.getAllMainOffice();
    ...
    swalWithBootstrapButtons
      ...
      .then((result) => {
        if (result.isConfirmed) {
          this.mainOfficeService.removeFromSdc(this.idMainOffice, id).subscribe(
            () => {
              this.getAllMainOffice();
